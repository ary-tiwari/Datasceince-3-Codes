...
X = pd.DataFrame(X_train.drop('Rings', axis=1))
Y = X_train["Rings"]
RMSE = []
for p in P:
...
for p in P:
	...
	test_poly = poly_features.fit_transform(pd.DataFrame(X_test.drop('Rings', axis=1)))
...
plt.scatter(Y_test, regressor.predict(poly_features.fit_transform(pd.DataFrame(X_test.drop('Rings', axis=1)))))
